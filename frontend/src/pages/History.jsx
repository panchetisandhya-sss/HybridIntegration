  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        const res = await fetch(`http://127.0.0.1:8000/api/history?token=${token}`);
              ...
              {history.map((entry, i) => {
                ...
                return (
                  <motion.tr
                    ...
                    className={`border-t border-white/5 ${secure ? "bg-emerald-500/5" : "bg-red-500/5"
                      }`}
                  >
                    ...
                    <td className="px-6 py-4 font-mono text-xs text-slate-500 overflow-hidden text-ellipsis max-w-[120px]" title={entry.hash}>
                      {entry.hash?.substring(0, 16)}...
                    </td>

                    <td className="px-6 py-4">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold tracking-wide ${secure
